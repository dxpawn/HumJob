...
natural-language editing, whole-melody reorganisation, and reharmonisation; off
by default, opt-in, text or numeric summaries only \\
...
A note on which backend the harness runs. The synthetic measurements (note and
rhythm F1, the diagnostic corpus, and the synthetic metamorphic properties) run
the deterministic pyin backend: it needs no model download and is reproducible,
and because the research contribution is the segmenter and quantiser rather than
the pitch model (RQ2), running the simplest tracker isolates the stages under
test. The evidence that runs on the four real recorded hums (the reconstruction
round-trip, the realism calibration, and the metamorphic checks on real audio)
uses PESTO instead, the web application's default, so that half of the evaluation
exercises the exact path a real user gets. The synthetic F1 figures below are
therefore pyin results and should not be read as PESTO's accuracy on real voice.
...
on first use. The library default is pyin, which needs no extra install;
selecting a backend that is not installed raises an import error rather than
degrading silently, so a deployment must install whichever backend it configures
(the public demo ships PESTO). \\
\addlinespace
