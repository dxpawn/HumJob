...
weekly practice plan, natural-language score edits, and reharmonisation, which is
the only part of the system that ever leaves the user's machine.
...
report documents the whole toolkit, concentrates its evaluation on the
...
The system exposes four optional, opt-in features backed by an external
...
file and is never exposed to the browser. All four share a single transport
...
tokens before its answer, so the two structured features (editing and
reharmonisation) request a larger token budget than the prose features and can be
pointed at a faster model through configuration. Beyond these four calls, HumJob's
...
four opt-in, text-only AI features (coaching, a practice plan, natural-language
editing, and reharmonisation), each of which sends a compact summary and never
any audio.
