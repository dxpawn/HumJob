...
particular, the central finding of the transcription evaluation is a negative
one: the synthetic benchmark the transcriber was tuned against no longer
discriminates, while it is not yet reliable on real hummed input. I have chosen to
...
this tractable with two deliberate user-interface constraints. The user hums the
...
segmenter and a backend-agnostic consolidation stage address the dominant
...
evaluation tells the same story. Quantisation is perfect on-grid, stays high
under human timing jitter (mean rhythm accuracy $0.975$), and, once a
tempo-refinement stage was added so that the quantiser follows the tempo
actually performed, recovers a $5\%$ tempo error to $1.000$ as well, so the
rhythm half of the harness is now saturated too. The residual real-world risk is
the tempo detector itself, a single global-tempo estimator that is fragile on
real, variable-tempo humming beyond the band the refinement stage can absorb.
This report documents the whole toolkit, concentrates its evaluation on the
transcriber, and gives an honest account of what a saturated synthetic benchmark
can and cannot demonstrate, together with a concrete plan for obtaining real
ground truth.
...
The rhythm evaluation was the more useful of the two for longer, because it once
localised the dominant real-world failure to a single cause: sensitivity to tempo
error. That diagnosis drove the tempo-refinement stage, which now absorbs a
moderate tempo error and, with it, most of the ``wrong rhythm'' and
``over-split'' behaviour that a small tempo mismatch used to cause. The
consequence is that the rhythm half of the synthetic harness is now at or near
ceiling too, so it has largely stopped discriminating as well. What remains open
is not on the synthetic harness at all: it is the tempo detector's behaviour on
real humming that drifts beyond the refinement band (Section~\ref{sec:bpm}), and,
more fundamentally, the absence of any real ground truth to measure against.
...
melodies exists. Two concrete consequences follow. Both halves of the synthetic
benchmark, note detection and rhythm, are now saturated and can no longer drive
improvement. And the over-splitting
...
\addlinespace
Saturated synthetic benchmark & Both the note-detection F1 and, since the
tempo-refinement stage landed, the rhythm accuracy are at ceiling on all
fixtures and no longer discriminate. Planned remedy: harder fixtures and, above
all, real recordings. \\
\addlinespace
...
corpus exists to measure against. The rhythm evaluation once localised the
dominant real-world failure to tempo sensitivity, and that diagnosis drove a
tempo-refinement stage that follows the tempo actually performed; with it in
place the rhythm half of the synthetic harness is now saturated as well. The open
problem is therefore the same for both halves: with no labelled real corpus,
neither can speak to real-world accuracy, and the tempo detector remains fragile
on real humming that drifts beyond the refinement band.
...
honest evaluation that says clearly where that core stands: strong on a synthetic
benchmark that has stopped being informative, and not yet validated on real
input. The most valuable next step
