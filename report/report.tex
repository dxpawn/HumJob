...
chord, scale, and scale-degree recognition.
...
Everything runs locally, and no audio is ever sent to any third-party service. The single exception to local-only operation is an opt-in
...
voice, delivered as a web application of six tabs: the transcriber and its
interactive editor, a pitch finder, a transposer, a real-time monitor and vocal
trainer, a sing-along trainer with optional, opt-in AI coaching, and an ear
trainer. All of it runs on the user's own machine, with the single opt-in
coaching call as the one deliberate exception.
